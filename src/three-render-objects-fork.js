    ...
    state.container.addEventListener(
      ...
      (ev) => {
        ...
        requestAnimationFrame(() => {
          ...
          if (ev.button === 0) {
            // left-click
            state.onClick(state.hoverObj || null, state.intersection, ev) // trigger background clicks with null
          }

          if (ev.button === 2 && state.onRightClick) {
            // right-click
            state.onRightClick(state.hoverObj || null, state.intersection, ev)
          }
        })
      },
