...
export interface ThreeRenderObjectsGenericInstance<ChainableInstance> {
  ...
  zoomToFit(
    durationMs?: number,
    padding?: number,
    objFilter?: (obj: Object3D) => boolean
  ): ChainableInstance
  fitToBbox(
  ...
  // Utility
  getBbox(objFilter?: (obj: Object3D) => boolean): {
    x: [number, number]
    y: [number, number]
    z: [number, number]
  }
...
export type ThreeRenderObjectsInstance =
  ThreeRenderObjectsGenericInstance<ThreeRenderObjectsInstance>

declare function ThreeRenderObjects(
